...
from shutil import copyfile, copytree, rmtree, move, copyfile

PATH_DATASET = r'E:\nanguozheng\pytorch-yolov4-deepsort\data'  # 需要处理的文件夹
PATH_NEW_DATASET = r'E:\nanguozheng\pytorch-yolov4-deepsort\deep_sort\data'  # 处理后的文件夹
                ...
                for root, _, files in os.walk(images_path + '/' + dir, topdown=True):
                    for j, file in enumerate(files):
                        if (j == 0):  # test dataset；每个车辆的第一幅图片
                            ...
                            src_path1 = root + '/' + file
                            dst_dir1 = train_save_path + '/' + dir
                            if not os.path.isdir(dst_dir1):
                                os.mkdir(dst_dir1)
                            dst_path1 = dst_dir1 + '/' + file
                            move(src_path1, dst_path1)
                            print("序号：%s  文件夹： %s  图片：%s 归为訓練集" % (i + 1, root, file))

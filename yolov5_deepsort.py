...
from yolo import YOLO as build_detector
...
class VideoTracker(object):
    ...
    def __enter__(self):
        ...
        if self.args.save_path:
            ...
            self.save_video_path = os.path.join(self.args.save_path, "results09.mp4")
            self.save_results_path = os.path.join(self.args.save_path, "results09.txt")
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if ref is True:
                ...
                if type(self.detector.new_detect(frame)) !=  type(frame):
                    bbox_xywh, cls_conf, cls_ids, = self.detector.new_detect(frame)

                    if cls_conf is not None and cls_ids is not None and bbox_xywh is not None:
                        ...
                        # do tracking
                        outputs = self.deepsort.update(new_bbox,cls_ids,cls_conf,im)
                        ...
                        if len(outputs) > 0:
                            ...
                            ori_im = draw_boxes(ori_im, bbox_xyxy, identities,cls)
...
if __name__ == "__main__":
    ...
    with VideoTracker( args, video_path='./1245.mp4') as vdo_trk:
        vdo_trk.run()
